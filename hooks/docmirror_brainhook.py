...
def _runtime_file(name, *fallbacks):
    """Resolve a plugin runtime file PLUGIN-NATIVELY: ${CLAUDE_PLUGIN_ROOT}/<name> (set by Claude Code
    when this runs as a registered plugin hook), else the first existing ~/.claude fallback (the
    hand-scatter transition path — the live scatter hook has no CLAUDE_PLUGIN_ROOT, so it still reads
    ~/.claude). Returns the first existing candidate, else the preferred (plugin) path."""
    pr = os.environ.get("CLAUDE_PLUGIN_ROOT")
    cands = ([Path(pr) / name] if pr else []) + list(fallbacks)
    for c in cands:
        try:
            if c.exists():
                return c
...
PROMPT_FILE = _runtime_file("docmirror_loop_prompt.txt",
                            Path.home() / ".claude" / "docmirror_loop_prompt.txt")
HARVEST_REMINDER_FILE = _runtime_file("docmirror_harvest_reminder.txt",
                                      Path.home() / ".claude" / "docmirror_harvest_reminder.txt")
...
# Plugin-native first; fallbacks = the (to-be-dissolved) doc-mirror skill templates dir, then a ~/.claude copy.
JOURNAL_REMINDER_FILE = _runtime_file(
    "docmirror_journal_reminder.txt",
    Path.home() / ".claude" / "skills" / "doc-mirror" / "templates" / "journal_reminder.txt",
    Path.home() / ".claude" / "docmirror_journal_reminder.txt")
...
def get_journal_reminder() -> str:
    """The journal-types reminder — appended EVERY blocked turn (resolved plugin-native by
    _runtime_file above). Empty string if it doesn't exist (fail-open)."""
    try:
        if JOURNAL_REMINDER_FILE.exists():
            return JOURNAL_REMINDER_FILE.read_text().strip()
    except Exception as e:
